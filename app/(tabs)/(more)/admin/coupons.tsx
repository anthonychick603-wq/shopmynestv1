...
// v1.0.92 (Build #10) — admin/site-wide coupons list. Coupons created here
// have no `_tnm_seller_id` meta, so they apply across every seller's cart.
...
export default function AdminCouponsScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await nest.listAdminCoupons();
      setItems(res.items || []);
    } catch (e) {
  ...
  const onDelete = (c: NestCoupon) => {
    ...
    (async () => {
      try {
        await nest.deleteAdminCoupon(c.id);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => { haptics.tap(); safeBack(router, "/admin"); }} style={styles.iconBtn} accessibilityLabel="Back">
          <Ionicons name="chevron-back" size={22} color={colors.onSurface} />
        </TouchableOpacity>
        <Text style={styles.title}>Site-wide coupons</Text>
        <CartHeaderButton />
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              title="No site coupons"
              message="Site-wide coupons apply across every seller. Sellers can still create their own shop coupons."
              actionLabel="Create coupon"
              onAction={() => router.push({ pathname: "/seller/coupon-edit", params: { scope: "admin" } } as never)}
            />
          }
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.card}
              onPress={() => { haptics.tap(); router.push({ pathname: "/seller/coupon-edit", params: { id: String(item.id), scope: "admin" } } as never); }}
